            ...
            if distance_to_target <= tolerance:
                for player in players:
                    if player.player_id == self.player_id:
                        ...
                        multiplier = max(0.0, 1.0 - (0.1 * over_limit))  # Linear 10% reduction per unit over limit
...
# Cow class
# Cow class (defined before game loop to ensure availability)
class Cow(Unit):
    milk_cost = 400
    wood_cost = 0
    returning = False
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                ...
                # Check grid buttons for spawning units or initiating building placement
                    ...
                    if not grid_button_clicked and VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                        ...
                        if placing_building:
                            ...
                            # Adjust tile range to cover the building's footprint
                            ...
                            if valid_placement:
                                ...
                                current_player.barns = [unit for unit in current_player.units if isinstance(unit, Barn)]
                                # Place dirt tiles under the building
                                for row in range(tile_y - building_size_tiles // 2, tile_y + building_size_tiles // 2 + 1):
                                    for col in range(tile_x - building_size_tiles // 2, tile_x + building_size_tiles // 2 + 1):
                                        if 0 <= row < GRASS_ROWS and 0 <= col < GRASS_COLS:
                                            grass_tiles[row][col] = Dirt(col * TILE_SIZE, row * TILE_SIZE)
                ...
                if placing_building:
                    ...
                    for unit in all_units:
                        if unit.selected and not isinstance(unit, (Building, Tree)):
                            ...
                            elif clicked_unit and (clicked_unit.player_id != unit.player_id or clicked_unit.player_id == 0):
                                unit.target = Vector2(click_pos.x, click_pos.y)
                                unit.autonomous_target = False
                                if isinstance(unit, Cow):
                                    unit.returning = False
                                highlight_times[clicked_unit] = current_time
                                print(f"Highlighting unit {clicked_unit.__class__.__name__} at {clicked_unit.pos} for Player {clicked_unit.player_id}")
                            else:
                                unit.target = Vector2(click_pos.x, click_pos.y)
                                unit.autonomous_target = False
                                if isinstance(unit, Cow):
            ...
            if placing_building:
                mouse_pos = Vector2(event.pos)
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    # Center the building preview on the mouse cursor
    ...
    if placing_building and building_pos:
        ...
        # Validate placement based on snapped position
        ...
        # Draw border at snapped position for clarity
