...
pygame.display.set_caption("Simple RTS Game with Cow Image")
...
SCROLL_SPEED = 10
SCROLL_MARGIN = 20
ZOOM_SCALE = 1.0  # New: 2x zoom factor
...
class GrassTile(SimpleTile):
    ...
    def draw(self, screen, camera_x, camera_y):
        ...
        pygame.draw.rect(screen, color, (self.pos.x * ZOOM_SCALE - camera_x, self.pos.y * ZOOM_SCALE - camera_y, TILE_SIZE * ZOOM_SCALE, TILE_SIZE * ZOOM_SCALE))
...
class Dirt(SimpleTile):
    ...
    def draw(self, screen, camera_x, camera_y):
        pygame.draw.rect(screen, GRASS_BROWN, (self.pos.x * ZOOM_SCALE - camera_x, self.pos.y * ZOOM_SCALE - camera_y, TILE_SIZE * ZOOM_SCALE, TILE_SIZE * ZOOM_SCALE))
...
class Tree(SimpleTile):
    ...
    def draw(self, screen, camera_x, camera_y):
        pygame.draw.rect(screen, TREE_GREEN, (self.pos.x * ZOOM_SCALE - camera_x, self.pos.y * ZOOM_SCALE - camera_y, TILE_SIZE * ZOOM_SCALE, TILE_SIZE * ZOOM_SCALE))
...
class Unit:
    ...
    def draw(self, screen, camera_x, camera_y):
        if not self.image:
            ...
            pygame.draw.rect(screen, color, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE))
            circle_radius = self.size * ZOOM_SCALE / 8
            pygame.draw.circle(screen, self.player_color, (int(self.pos.x * ZOOM_SCALE - camera_x), int(self.pos.y * ZOOM_SCALE - camera_y)), circle_radius)
        else:
            scaled_image = pygame.transform.scale(self.image, (int(self.size * ZOOM_SCALE), int(self.size * ZOOM_SCALE)))
            image_rect = scaled_image.get_rect(center=(int(self.pos.x * ZOOM_SCALE - camera_x), int(self.pos.y * ZOOM_SCALE - camera_y)))
            screen.blit(scaled_image, image_rect)
        if self.selected:
            pygame.draw.rect(screen, self.player_color, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), 1)
...
class Barn(Building):
    ...
    def draw(self, screen, camera_x, camera_y):
        color = GREEN if self.selected else self.color
        barn_surface = pygame.Surface((self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), pygame.SRCALPHA)
        pygame.draw.rect(barn_surface, color, (0, 0, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE))
        screen.blit(barn_surface, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y))
        circle_radius = self.size * ZOOM_SCALE / 8
        pygame.draw.circle(screen, self.player_color, (int(self.pos.x * ZOOM_SCALE - camera_x), int(self.pos.y * ZOOM_SCALE - camera_y)), circle_radius)
        if self.selected:
            pygame.draw.rect(screen, self.player_color, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), 1)
...
class TownCenter(Building):
    ...
    def draw(self, screen, camera_x, camera_y):
        color = GREEN if self.selected else self.color
        town_center_surface = pygame.Surface((self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), pygame.SRCALPHA)
        pygame.draw.rect(town_center_surface, color, (0, 0, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE))
        screen.blit(town_center_surface, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y))
        circle_radius = self.size * ZOOM_SCALE / 8
        pygame.draw.circle(screen, self.player_color, (int(self.pos.x * ZOOM_SCALE - camera_x), int(self.pos.y * ZOOM_SCALE - camera_y)), circle_radius)
        if self.selected:
            pygame.draw.rect(screen, self.player_color, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), 1)
...
class Cow(Unit):
    def __init__(self, x, y, player_id, player_color):
        ...
        try:
            ...
            self.image = pygame.transform.scale(self.image, (int(self.size * ZOOM_SCALE), int(self.size * ZOOM_SCALE)))
    ...
    def draw(self, screen, camera_x, camera_y):
        if not self.image:
            ...
            image_rect = self.image.get_rect(center=(int(self.pos.x * ZOOM_SCALE - camera_x), int(self.pos.y * ZOOM_SCALE - camera_y)))
            screen.blit(self.image, image_rect)
        if self.selected:
            pygame.draw.rect(screen, self.player_color, (self.pos.x * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_x, self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - camera_y, self.size * ZOOM_SCALE, self.size * ZOOM_SCALE), 1)
            # Draw health bar
        bar_width = 16 * ZOOM_SCALE
        bar_height = 4 * ZOOM_SCALE
        bar_offset = 2 * ZOOM_SCALE
        bar_x = self.pos.x * ZOOM_SCALE - bar_width / 2 - camera_x
        bar_y = self.pos.y * ZOOM_SCALE - self.size * ZOOM_SCALE / 2 - bar_height - bar_offset - camera_y
            ...
            if not self.target:
                ...
                if available_barns:
                    ...
                    barn_corners = [
                        ...
                        Vector2(closest_barn.pos.x - closest_barn.size / 2, closest_barn.pos.y + closest_barn.size / 2),  # Bottom-left
                        Vector2(closest_barn.pos.x + closest_barn.size / 2, closest_barn.pos.y + closest_barn.size / 2)  # Bottom-right
                    ]
...
while running:
    ...
    if keys[pygame.K_RIGHT]:
        camera_x = min(MAP_WIDTH * ZOOM_SCALE - VIEW_WIDTH, camera_x + SCROLL_SPEED)
    if keys[pygame.K_UP]:
        camera_y = max(0, camera_y - SCROLL_SPEED)
    if keys[pygame.K_DOWN]:
        camera_y = min(MAP_HEIGHT * ZOOM_SCALE - VIEW_HEIGHT, camera_y + SCROLL_SPEED)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    if VIEW_X <= mouse_pos.x <= VIEW_X + VIEW_WIDTH and VIEW_Y <= mouse_pos.y <= VIEW_Y + VIEW_HEIGHT:
                        click_pos = Vector2((mouse_pos.x - VIEW_X) / ZOOM_SCALE + camera_x / ZOOM_SCALE, (mouse_pos.y - VIEW_Y) / ZOOM_SCALE + camera_y / ZOOM_SCALE)
                        unit_clicked = None
                        for unit in all_units:
                            if current_player and unit.player_id == current_player.player_id and unit.is_clicked(Vector2((mouse_pos.x - VIEW_X) / ZOOM_SCALE, (mouse_pos.y - VIEW_Y) / ZOOM_SCALE), camera_x / ZOOM_SCALE, camera_y / ZOOM_SCALE):
                                unit_clicked = unit
                                break
                        if unit_clicked:
                            for player in players:
                    ...
                    if VIEW_X <= mouse_pos.x <= VIEW_X + VIEW_WIDTH and VIEW_Y <= mouse_pos.y <= VIEW_Y + VIEW_HEIGHT:
                        for unit in all_units:
                            if unit.selected and not isinstance(unit, Building):
                                unit.target = Vector2((mouse_pos.x - VIEW_X) / ZOOM_SCALE + camera_x / ZOOM_SCALE, (mouse_pos.y - VIEW_Y) / ZOOM_SCALE + camera_y / ZOOM_SCALE)
                ...
                if event.button == 1 and selecting and current_player is not None:
                    mouse_pos = Vector2(event.pos)
                    if VIEW_X <= mouse_pos.x <= VIEW_X + VIEW_WIDTH and VIEW_Y <= mouse_pos.y <= VIEW_Y + VIEW_HEIGHT:
                        selection_end = Vector2((mouse_pos.x - VIEW_X) / ZOOM_SCALE + camera_x / ZOOM_SCALE, (mouse_pos.y - VIEW_Y) / ZOOM_SCALE + camera_y / ZOOM_SCALE)
                        selecting = False
                        for player in players:
                            if player.player_id == current_player.player_id:
                                for unit in player.units:
    ...
    start_col = int((camera_x / ZOOM_SCALE) // TILE_SIZE)
    end_col = min(GRASS_COLS, int((camera_x / ZOOM_SCALE + VIEW_WIDTH / ZOOM_SCALE) // TILE_SIZE) + 1)
    start_row = int((camera_y / ZOOM_SCALE) // TILE_SIZE)
    end_row = min(GRASS_ROWS, int((camera_y / ZOOM_SCALE + VIEW_HEIGHT / ZOOM_SCALE) // TILE_SIZE) + 1)
    for row in range(start_row, end_row):
        for col in range(start_col, end_col):
            grass_tiles[row][col].draw(screen, camera_x - VIEW_X * ZOOM_SCALE, camera_y - VIEW_Y * ZOOM_SCALE)

    # Draw selection rectangle
    if selecting and selection_start and selection_end and current_player is not None:
        rect = pygame.Rect(
            min(selection_start.x * ZOOM_SCALE - camera_x + VIEW_X, selection_end.x * ZOOM_SCALE - camera_x + VIEW_X),
            min(selection_start.y * ZOOM_SCALE - camera_y + VIEW_Y, selection_end.y * ZOOM_SCALE - camera_y + VIEW_Y),
            abs(selection_end.x * ZOOM_SCALE - selection_start.x * ZOOM_SCALE),
            abs(selection_end.y * ZOOM_SCALE - selection_start.y * ZOOM_SCALE)
        )
    ...
    # Draw units
    for unit in all_units:
        unit.draw(screen, camera_x - VIEW_X * ZOOM_SCALE, camera_y - VIEW_Y * ZOOM_SCALE)
