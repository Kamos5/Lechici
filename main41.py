...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                ...
                # Check grid buttons for spawning units or initiating building placement
                    ...
                    if not grid_button_clicked and VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                        ...
                        if placing_building:
                            ...
                            building_pos = Vector2(tile_x * TILE_SIZE + building_size / 2, tile_y * TILE_SIZE + building_size / 2)
            ...
            if placing_building:
                mouse_pos = Vector2(event.pos)
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    tile_x = int((mouse_pos.x - VIEW_MARGIN_LEFT + camera_x) // TILE_SIZE)
                    tile_y = int((mouse_pos.y - VIEW_MARGIN_TOP + camera_y) // TILE_SIZE)
                    building_pos = Vector2(tile_x * TILE_SIZE + building_size / 2, tile_y * TILE_SIZE + building_size / 2)
    ...
    if placing_building and building_pos:
        ...
        pygame.draw.rect(screen, border_color, (x - building_size / 2, y - building_size / 2, building_size, building_size), 2)
