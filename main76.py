...
class Bridge(SimpleTile):
    ...
    def load_image(cls):
        try:
            ...
            print(f"Failed to load bridge.png: {e}")
            cls._image = None
    ...
    def draw(self, surface, camera_x, camera_y):
        ...
        if self._image:
            ...
            # Fallback: Draw a brown rectangle to represent a bridge
            pygame.draw.rect(surface, (139, 69, 19), (self.pos.x - camera_x, self.pos.y - camera_y, TILE_SIZE, TILE_SIZE))
...
def generate_river(grass_tiles, tile_size, rows, cols):
    """
    Generate a 3-tile-wide squiggly river from bottom-left to top-right with an island.
    Args:
    ...
    for i in range(num_points + 1):
        ...
        # Add squiggly offset
        amplitude = 4  # Max deviation in tiles
        frequency = 0.12  # Moderate waviness
    ...
    for idx, (col, row) in enumerate(path):
        # Add central tile and surrounding tiles to ensure continuity
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                tile_row = row + dr
                tile_col = col + dc
        ...
        # Ensure connectivity between path points
        if idx < len(path) - 1:
