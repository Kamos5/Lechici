    ...
    def manage_unit_production(self):
        ...
        if barracks and self.player.unit_count < self.max_units:
            ...
            if self.player.wood < 500 and self.player.milk >= Axeman.milk_cost and self.player.wood >= Axeman.wood_cost:
                ...
                print(f"AI: Queued Axeman production in Barracks at {barracks.pos} for Player {self.player.player_id}")
            elif self.player.milk >= Archer.milk_cost and self.player.wood >= Archer.wood_cost:
                self.production_queues[barracks] = {
                    'unit_type': Archer,
                    'start_time': current_time,
                    'player_id': self.player.player_id
                }
                print(f"AI: Queued Archer production in Barracks at {barracks.pos} for Player {self.player.player_id}")
            elif self.player.milk >= Knight.milk_cost and self.player.wood >= Knight.wood_cost:
                self.production_queues[barracks] = {
                    'unit_type': Knight,
                    'start_time': current_time,
                    'player_id': self.player.player_id
                }
                print(f"AI: Queued Knight production in Barracks at {barracks.pos} for Player {self.player.player_id}")
