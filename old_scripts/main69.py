...
class Knight(Unit):
    ...
    def __init__(self, x, y, player_id, player_color):
        super().__init__(x, y, size=16, speed=2.5, color=BLUE, player_id=player_id, player_color=player_color)
...
class Archer(Unit):
    ...
    def __init__(self, x, y, player_id, player_color):
        super().__init__(x, y, size=16, speed=2.3, color=YELLOW, player_id=player_id, player_color=player_color)
...
class Cow(Unit):
    ...
    def __init__(self, x, y, player_id, player_color):
        super().__init__(x, y, size=16, speed=2, color=BROWN, player_id=player_id, player_color=player_color)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if current_player and not clicked_something:
                    ...
                    if not grid_button_clicked and VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                        # Check for any unit/building to select (including neutral/enemy)
                        unit_clicked = None
                        for unit in all_units:
                        ...
                        if unit_clicked:
                            if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                                # Toggle selection for any unit (including neutral/enemy)
                                ...
                                # Only set building selection for current player's buildings
                                ...
                                # Allow targeting for current player's selected units
                            ...
                            # Start selection rectangle if nothing was clicked and not placing a building
                            if not placing_building:
                                ...
                                building_size_tiles = int(building_to_place.size // TILE_SIZE)
            ...
            if event.button == 1 and selecting and current_player:
                ...
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    ...
                    for player in players:
                        player.deselect_all_units()
                    # Allow selection rectangle to select current player's units only
                    for unit in current_player.units:
