        ...
        if self.wave_number == 0 and elapsed_time >= 150:
            ...
            if archers:
                ...
                if enemy_buildings:
                    ...
                    archer.path = waypoint_graph.get_path(archer.pos, target_building.pos, archer)
...
def generate_river(grass_tiles, tile_size, rows, cols):
    """
    Generate a 3-tile-wide squiggly river from bottom-left to top-right with up to 3 dirt bridges,
    each 2 tiles wide, extending to grass tiles (length 5-13 tiles), separated by at least 10 tiles.
    Args:
    ...
    # Find up to 3 straight river sections for bridges
    bridge_width = 2
    min_bridge_length = river_width + 2  # 5 tiles
    max_bridge_length = river_width + 5  # 8 tiles
    min_separation = 10  # Min tiles between bridge centers
    bridge_centers = []
    straight_sections = []

    # Calculate straightness for each path segment
    for idx in range(1, len(path) - 1):
        ...
        straight_sections.append((angle, idx, p2))

    # Sort by straightness (smallest angle first)
    straight_sections.sort()

    # Select up to 3 non-overlapping, separated sections
    for angle, idx, center in straight_sections:
        if len(bridge_centers) >= 3:
            break
        # Check separation from existing bridges
        too_close = False
        for existing_idx, _ in bridge_centers:
            if abs(idx - existing_idx) < min_separation:
                too_close = True
                break
        if not too_close:
            bridge_centers.append((idx, center))

    print(f"Found {len(bridge_centers)} suitable bridge locations")

    # Place bridges
    bridge_tiles = set()
    for bridge_idx, (path_idx, (col, row)) in enumerate(bridge_centers):
        # Determine orientation
        p1 = path[max(0, path_idx - 1)]
        p3 = path[min(len(path) - 1, path_idx + 1)]
        river_dx = p3[0] - p1[0]
        river_dy = p3[1] - p1[1]
        is_horizontal = abs(river_dx) > abs(river_dy)

        # Extend bridge to touch grass
        bridge_length = min_bridge_length  # Start with minimum
        found_left = False
        found_right = False
        max_extension = max_bridge_length - river_width

        if is_horizontal:
            # Check left (decreasing col)
            for dc in range(0, -max_extension - 1, -1):
                check_col = col + dc
                if check_col < 0:
                    break
                if not any((row + dr, check_col) in river_tiles for dr in [-1, 0, 1]):
                    found_left = True
                    bridge_length = max(bridge_length, abs(dc) + 1)
                    break
            # Check right (increasing col)
            for dc in range(0, max_extension + 1):
                check_col = col + dc
                if check_col >= cols:
                    break
                if not any((row + dr, check_col) in river_tiles for dr in [-1, 0, 1]):
                    found_right = True
                    bridge_length = max(bridge_length, dc + 1)
                    break
            # Place bridge tiles
            for dr in [-bridge_width // 2, bridge_width // 2 - 1]:
                for dc in range(-bridge_length, bridge_length + 1):
                    tile_row = row + dr
                    tile_col = col + dc
                    if 0 <= tile_row < rows and 0 <= tile_col < cols:
            ...
            for dr in range(0, -max_extension - 1, -1):
                check_row = row + dr
                if check_row < 0:
                    break
                if not any((check_row, col + dc) in river_tiles for dc in [-1, 0, 1]):
                    found_left = True
                    bridge_length = max(bridge_length, abs(dr) + 1)
                    break
            # Check down (increasing row)
            for dr in range(0, max_extension + 1):
                check_row = row + dr
                if check_row >= rows:
                    break
                if not any((check_row, col + dc) in river_tiles for dc in [-1, 0, 1]):
                    found_right = True
                    bridge_length = max(bridge_length, dr + 1)
                    break
            # Place bridge tiles
            for dc in [-bridge_width // 2, bridge_width // 2 - 1]:
                for dr in range(-bridge_length, bridge_length + 1):
                    tile_row = row + dr
                    tile_col = col + dc
                    if 0 <= tile_row < rows and 0 <= tile_col < cols:
                        bridge_tiles.add((tile_row, tile_col))

        print(f"Bridge {bridge_idx + 1} at ({row}, {col}), horizontal={is_horizontal}, length={2 * bridge_length + 1}, grass_reached={found_left and found_right}")
    ...
    print(f"Added {len(bridge_tiles)} bridge tiles across {len(bridge_centers)} bridges")
