...
class Axeman(Unit):
    ...
    def __init__(self, x, y, player_id, player_color):
        ...
        self.chop_damage = 1
        self.special = 0
        self.return_pos = None  # Store position where tree was chopped
        self.depositing = False  # Flag to track if Axeman is depositing wood

    def move(self, units, spatial_grid):
        self.velocity = Vector2(0, 0)
        if self.special >= 100 and not self.depositing:
            # Find closest TownCenter owned by this player
            ...
            # Move to TownCenter to deposit wood
            direction = self.target - self.pos
            distance_to_target = direction.length()
            # Find the TownCenter at the target position
            ...
            if distance_to_target <= tolerance:  # Use combined size for TownCenter
                # Deposit wood
                ...
                self.special = 0
                self.depositing = False
                self.target = self.return_pos  # Return to position where tree was chopped
                print(f"Axeman at {self.pos} returning to {self.return_pos}")
            else:
                self.velocity = direction.normalize() * self.speed
        elif self.return_pos and self.target == self.return_pos:
            # Move back to return_pos
            direction = self.target - self.pos
            distance_to_target = direction.length()
            if distance_to_target <= self.size:  # Use Axeman's size for return_pos
                self.pos = Vector2(self.return_pos)
                self.target = None
                self.return_pos = None
                # Find closest tree to harvest using spatial grid
    ...
    def chop_tree(self, trees):
        if self.special > 0:  # Only chop trees if special is 0
            return
        for tree in trees:
            if (isinstance(tree, Tree) and tree.player_id == 0 and
                ...
                if tree.hp <= 0:
                    self.return_pos = Vector2(self.pos)  # Store position where tree was chopped
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                # Check game view clicks only if no button was clicked
