...
# Move order tracking
move_order_times = {}  # Tracks tile positions (as tuples) and timestamps for move orders
...
class Tree(Unit):
    ...
    _color_index = 0
    _colors = [RED, GREEN, BLUE, YELLOW]  # Cycle through these colors
    ...
    def __init__(self, x, y, size, color, player_id, player_color):
        ...
        self.hp = 180  # Initial health (3 seconds at 60 FPS with 1 damage per frame)
    ...
    def draw(self, screen, camera_x, camera_y, axemen_targets):
        ...
        if self._image:
            ...
            # Apply tint only if this tree is targeted by an Axeman
            if any(self.pos.distance_to(axeman.pos) <= TILE_SIZE and axeman.target == self.pos for axeman in axemen_targets):
                ...
                mask_surface.fill(tint_color + (128,))  # Semi-transparent tint
                tinted_image.blit(mask_surface, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
            screen.blit(tinted_image, image_rect)
            # Draw white border if targeted by an Axeman
            if any(axeman.target == self.pos for axeman in axemen_targets):
                pygame.draw.rect(screen, WHITE, (self.pos.x - TILE_SIZE / 2 - camera_x, self.pos.y - TILE_SIZE / 2 - camera_y, TILE_SIZE, TILE_SIZE), 1)
            if self._selected:
                pygame.draw.rect(screen, YELLOW, (self.pos.x - TILE_SIZE / 2 - camera_x, self.pos.y - TILE_SIZE / 2 - camera_y, TILE_SIZE, TILE_SIZE), 1)
        else:
...
class Axeman(Unit):
    def __init__(self, x, y, player_id, player_color):
        ...
        self.chop_damage = 1  # Damage per frame when near a targeted tree
    ...
    def chop_tree(self, trees):
        for tree in trees:
            if (isinstance(tree, Tree) and tree.player_id == 0 and
                self.target == tree.pos and self.pos.distance_to(tree.pos) <= TILE_SIZE):  # Chop only if targeting the tree
                ...
                if tree.hp <= 0:
                    ...
                    self.target = None  # Clear target after chopping
                break
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.button == 3:  # Right-click to move or target tree
                mouse_pos = Vector2(event.pos)
                if VIEW_X <= mouse_pos.x <= VIEW_X + VIEW_WIDTH and VIEW_Y <= mouse_pos.y <= VIEW_Y + VIEW_HEIGHT:
                    click_pos = Vector2(mouse_pos.x - VIEW_X + camera_x, mouse_pos.y - VIEW_Y + camera_y)
                    ...
                    for unit in all_units:
                        if unit.selected and not isinstance(unit, (Building, Tree)):
                            clicked_tree = next((tree for tree in all_units if isinstance(tree, Tree) and tree.is_clicked(Vector2(mouse_pos.x - VIEW_X, mouse_pos.y - VIEW_Y), camera_x, camera_y)), None)
                            if clicked_tree and isinstance(unit, Axeman):
                                unit.target = clicked_tree.pos  # Set target to tree position for chopping
                            else:
                                unit.target = Vector2(tile_pos[0], tile_pos[1])  # Move to clicked tile center for other units
                                move_order_times[tile_pos] = current_time  # Record move order time
    ...
    for row in range(start_row, end_row):
        for col in range(start_col, end_col):
            ...
            if tile_pos in move_order_times and current_time - move_order_times[tile_pos] <= 0.4:
                # Draw crossed white 'X' on the tile
                pygame.draw.line(screen, WHITE, (tile_pos[0] - TILE_SIZE // 4 - camera_x + VIEW_X, tile_pos[1] - TILE_SIZE // 4 - camera_y + VIEW_Y),
                                (tile_pos[0] + TILE_SIZE // 4 - camera_x + VIEW_X, tile_pos[1] + TILE_SIZE // 4 - camera_y + VIEW_Y), 1)
                pygame.draw.line(screen, WHITE, (tile_pos[0] + TILE_SIZE // 4 - camera_x + VIEW_X, tile_pos[1] - TILE_SIZE // 4 - camera_y + VIEW_Y),
                                (tile_pos[0] - TILE_SIZE // 4 - camera_x + VIEW_X, tile_pos[1] + TILE_SIZE // 4 - camera_y + VIEW_Y), 1)
    ...
    for unit in all_units:
        if isinstance(unit, Tree):
            unit.draw(screen, camera_x - VIEW_X, camera_y - VIEW_Y, axemen)
        else:
