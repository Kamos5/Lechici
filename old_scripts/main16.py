...
BUTTON_PLAYER1_POS = (SCREEN_WIDTH // 2 - BUTTON_WIDTH - BUTTON_MARGIN - 50, PANEL_Y + 10 + 1 * (BUTTON_HEIGHT + BUTTON_MARGIN))
    ...
    def resolve_collisions(self, units):
        for other in units:
            if other is not self:
                # Special collision logic for Cow entering a Barn via nearest corner
                if isinstance(self, Cow) and isinstance(other, Barn):
                    ...
                    corner_radius = 10
                    # If Cow is too close to the corner and is not already targeting it, push it away
                ...
                # Cows avoiding collisions with non-barn buildings
                ...
                # Non-cow units avoiding buildings
                ...
                # All non-building units avoid each other
                ...
                # Buildings don't move, so skip resolving their own collisions
...
while len(tree_objects) < target_tree_count and eligible_tiles:
    ...
    if patch:
        for r, c in patch:
            tree_objects.append(Tree(c * TILE_SIZE + TILE_SIZE / 2, r * TILE_SIZE + TILE_SIZE / 2, 20, GRAY, 0, GRAY))
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if VIEW_X <= mouse_pos.x <= VIEW_X + VIEW_WIDTH and VIEW_Y <= mouse_pos.y <= VIEW_Y + VIEW_HEIGHT:
                    ...
                    for player in players:
                        player.deselect_all_units()
                    print("Selected Player 0; only Player 0 units can be selected")
