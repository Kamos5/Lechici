...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    ...
                    for player in players:
                        player.deselect_all_units()
                elif barn_selected and current_player and current_player.milk >= 500 and button_spawn_0.collidepoint(event.pos):
    ...
    # Draw 1x3 grid of buttons
    for row in range(GRID_BUTTON_ROWS):
        for col in range(GRID_BUTTON_COLS):
            button_rect = grid_buttons[row][col]
            if row == 0 and col == 0 and barn_selected and current_player:
                # Draw spawn cow button in top-left cell
                if current_player.milk >= 500:
                ...
                screen.blit(spawn_cow_text, (button_rect.x + 60, button_rect.y + 5))
                screen.blit(spawn_cow_text, (button_rect.x + 60, button_rect.y + 20))
            else:
                # Draw empty button
