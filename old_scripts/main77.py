...
def generate_river(grass_tiles, tile_size, rows, cols):
    """
    Generate a 3-tile-wide straighter river from bottom-left to top-right with 3-tile-wide bridges
    that extend at least one tile over grass or dirt on both ends, with ends sharing x or y values.
    If no bridges are found, increase bridge length until at least one is placed.
    Args:
    ...
    river_tiles = set()
    bridge_locations = []  # Store the two best bridge locations
    ...
    noise = PerlinNoise(octaves=2, seed=random.randint(0, 1000000))  # Smoother path
    ...
    for step in range(steps):
        ...
        current_x += x_step + noise_value * min(cols, rows) / 50
        current_y += y_step + noise_value * min(cols, rows) / 50
    ...
    # Place River tiles along the path
    ...
    valid_spans = []
    bridge_min_length = 5  # Start with minimum length (land-3 river-land)
    bridge_max_length = 10  # Maximum length to try
    min_bridge_distance = 5  # Minimum distance between bridge centers

    # Try increasing bridge lengths until at least one span is found
    for length in range(bridge_min_length, bridge_max_length + 1):
        ...
        if valid_spans:
            break  # Stop if at least one span is found
        print(f"No valid spans found for length {length}")

    # Select two spans with minimum distance, or one if that's all we have
    selected_spans = []
    if valid_spans:
        valid_spans.sort(key=lambda x: x['score'])
        selected_spans.append(valid_spans[0])  # Take the best span
        # Add second span if it's far enough from the first
        for span in valid_spans[1:]:
            dist = min(abs(span['center'][0] - selected_spans[0]['center'][0]),
                       abs(span['center'][1] - selected_spans[0]['center'][1]))
            if dist >= min_bridge_distance:
                selected_spans.append(span)
                break
    print(f"Found {len(valid_spans)} valid spans, selected {len(selected_spans)}")
    ...
    for span in selected_spans:
        print(f"Bridge {span['direction']} at {span['center']}: tiles {span['tiles']}")
