...
move_order_times = {}
highlight_times = {}  # Tracks units/buildings to highlight with white borders
        ...
        for tree in trees:
            if (isinstance(tree, Tree) and tree.player_id == 0 and
                ...
                if tree.hp <= 0:
                    ...
                    players[0].remove_unit(tree)  # Use remove_unit
...
class Player:
    def __init__(self, player_id, color, start_x, start_y):
        ...
        self.cow_in_barn = {}
        self.barns = []
        self.unit_limit = None if player_id == 0 else 15  # No limit for Player 0
        self.unit_count = 0
        offset_x = start_x
        offset_y = start_y
        if self.player_id > 0:
            ...
            self.unit_count = len(self.units)  # Initialize with 7 units
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                # Check player selection buttons first
                if button_player0.collidepoint(event.pos):
                    ...
                    for player in players:
                        player.deselect_all_units()
                # Check grid buttons for spawning
                elif current_player:
                    grid_button_clicked = False
                    if barn_selected:
                        if grid_buttons[0][0].collidepoint(event.pos):
                            print("First grid button clicked for Barn")
                            if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                               current_player.milk >= Cow.milk_cost and current_player.wood >= Cow.wood_cost:
                                ...
                                if current_player.unit_limit is not None and current_player.unit_count >= current_player.unit_limit:
                                    print(f"Cannot spawn Cow: Player {current_player.player_id} has reached unit limit ({current_player.unit_count}/{current_player.unit_limit})")
                                else:
                                    print(f"Insufficient resources: milk={current_player.milk}/{Cow.milk_cost}, wood={current_player.wood}/{Cow.wood_cost}")
                        ...
                        if selected_barracks:
                            if grid_buttons[0][0].collidepoint(event.pos):
                                ...
                                if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                   current_player.milk >= Axeman.milk_cost and current_player.wood >= Axeman.wood_cost:
                                    ...
                                    if current_player.unit_limit is not None and current_player.unit_count >= current_player.unit_limit:
                                        print(f"Cannot spawn Axeman: Player {current_player.player_id} has reached unit limit ({current_player.unit_count}/{current_player.unit_limit})")
                                    else:
                                        print(f"Insufficient resources: milk={current_player.milk}/{Axeman.milk_cost}, wood={current_player.wood}/{Axeman.wood_cost}")
                            elif grid_buttons[1][0].collidepoint(event.pos):
                                print("Second grid button clicked for Barracks")
                                if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                   current_player.milk >= Archer.milk_cost and current_player.wood >= Archer.wood_cost:
                                    ...
                                    if current_player.unit_limit is not None and current_player.unit_count >= current_player.unit_limit:
                                        print(f"Cannot spawn Archer: Player {current_player.player_id} has reached unit limit ({current_player.unit_count}/{current_player.unit_limit})")
                                    else:
                                        print(f"Insufficient resources: milk={current_player.milk}/{Archer.milk_cost}, wood={current_player.wood}/{Archer.wood_cost}")
                            elif grid_buttons[2][0].collidepoint(event.pos):
                                print("Third grid button clicked for Barracks")
                                if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                   current_player.milk >= Knight.milk_cost and current_player.wood >= Knight.wood_cost:
                                    ...
                                    if current_player.unit_limit is not None and current_player.unit_count >= current_player.unit_limit:
                                        print(f"Cannot spawn Knight: Player {current_player.player_id} has reached unit limit ({current_player.unit_count}/{current_player.unit_limit})")
                                    else:
                                        print(f"Insufficient resources: milk={current_player.milk}/{Knight.milk_cost}, wood={current_player.wood}/{Knight.wood_cost}")
                        else:
                            print("No selected Barracks found")
                    # Check unit icon clicks only if no grid button was clicked
    ...
    for row in range(GRID_BUTTON_ROWS):
        for col in range(GRID_BUTTON_COLS):
            ...
            if barn_selected and current_player and row == 0 and col == 0:
                spawn_button_color = HIGHLIGHT_GRAY if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                                     current_player.milk >= Cow.milk_cost and current_player.wood >= Cow.wood_cost else GRAY
                ...
                if row == 0 and col == 0:
                    spawn_button_color = HIGHLIGHT_GRAY if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                                         current_player.milk >= Axeman.milk_cost and current_player.wood >= Axeman.wood_cost else GRAY
                    pygame.draw.rect(screen, spawn_button_color, button_rect)
                    ...
                    spawn_button_color = HIGHLIGHT_GRAY if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                                         current_player.milk >= Archer.milk_cost and current_player.wood >= Archer.wood_cost else GRAY
                    pygame.draw.rect(screen, spawn_button_color, button_rect)
                    ...
                    spawn_button_color = HIGHLIGHT_GRAY if (current_player.unit_limit is None or current_player.unit_count < current_player.unit_limit) and \
                                                         current_player.milk >= Knight.milk_cost and current_player.wood >= Knight.wood_cost else GRAY
                    pygame.draw.rect(screen, spawn_button_color, button_rect)
    ...
    if current_player:
        ...
        if current_player.unit_limit is not None:
            ...
            unit_text_surface = font.render(unit_text, True, current_player.color)
