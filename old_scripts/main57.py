...
class PlayerAI:
    def __init__(self, player, grass_tiles, spatial_grid, all_units, production_queues):
        ...
        self.wave_start_time = 0  # Initialize to 0; will be set when update is called
        self.wave_number = 0  # Track current wave
        self.attack_units = []  # Units currently assigned to attack
        self.target_player_id = 1  # Target Player 1 (human player)
    ...
    def initiate_wave(self, current_time, waypoint_graph):
        ...
        if self.wave_start_time == 0:
            self.wave_start_time = current_time  # Set wave start time on first call
        ...
        if not self.target_camp_pos:
            ...
            if town_centers:
                ...
                self.target_camp_pos = Vector2(150, 150)  # Default to Player 1's starting position
        ...
        if self.wave_number == 0 and elapsed_time >= 30:
            ...
            if archers:
                ...
                self.wave_number = 1
                self.last_decision_time = current_time  # Reset decision timer to align with waves

        # Wave 2: After 90 seconds (30 + 60), 2 random military units
        ...
        # Wave 3: After 150 seconds (30 + 60 + 60), 5–10 random military units
    ...
    def update_attack_units(self, current_time, waypoint_graph):
        """Update attack units to engage closest enemy or return to camp if no targets."""
        target_player = next((p for p in players if p.player_id == self.target_player_id), None)
        if not target_player:
            return
        enemy_units = [u for u in target_player.units if u.hp > 0 and u.alpha == 255]
        for unit in self.attack_units[:]:
            ...
            if not unit.target or (isinstance(unit.target, Vector2) and unit.pos.distance_to(unit.target) <= unit.size):
                ...
                if closest_enemy:
                    ...
                    # If no enemies remain, return to camp
                    unit.target = self.target_camp_pos
                    unit.path = waypoint_graph.get_path(unit.pos, self.target_camp_pos, unit)
                    unit.path_index = 0
                    print(f"AI: {unit.__class__.__name__} at {unit.pos} returning to Player 1 camp at {self.target_camp_pos}")
            # Attack if in range
            if isinstance(unit.target, Unit) and unit.target in all_units and unit.target.hp > 0:
                unit.attack(unit.target, current_time)
                if unit.target.hp <= 0:
                    print(f"AI: {unit.__class__.__name__} at {unit.pos} destroyed {unit.target.__class__.__name__} at {unit.target.pos}")
                    unit.target = None  # Clear target to find a new one
            # Move towards target
            unit.move(self.all_units, self.spatial_grid, waypoint_graph)

    def update(self, current_time, waypoint_graph):
        """Update AI decisions including attack waves."""
...
while running:
    ...
    player2_ai.update(current_time, waypoint_graph)
    player2_ai.update_attack_units(current_time, waypoint_graph)
