...
class SimpleTile:
    ...
    def draw(self, screen, camera_x, camera_y):
        if (self.pos.x < camera_x - TILE_SIZE or self.pos.x > camera_x + VIEW_WIDTH or
            ...
            return
        if self._image:
            screen.blit(self._image, (self.pos.x - camera_x + VIEW_MARGIN_LEFT, self.pos.y - camera_y + VIEW_MARGIN_TOP))
        else:
            pygame.draw.rect(screen, GRASS_BROWN, (self.pos.x - camera_x + VIEW_MARGIN_LEFT, self.pos.y - camera_y + VIEW_MARGIN_TOP, TILE_SIZE, TILE_SIZE))
...
class GrassTile(SimpleTile):
    ...
    def draw(self, screen, camera_x, camera_y):
        ...
        if self._grass_image and self._dirt_image:
            ...
            screen.blit(blended_surface, (self.pos.x - camera_x + VIEW_MARGIN_LEFT, self.pos.y - camera_y + VIEW_MARGIN_TOP))
        else:
            color = (
            ...
            pygame.draw.rect(screen, color, (self.pos.x - camera_x + VIEW_MARGIN_LEFT, self.pos.y - camera_y + VIEW_MARGIN_TOP, TILE_SIZE, TILE_SIZE))
...
class Unit:
    ...
    def draw(self, screen, camera_x, camera_y):
        ...
        if not image:
            ...
            pygame.draw.rect(screen, color, (self.pos.x - self.size / 2 - camera_x + VIEW_MARGIN_LEFT, self.pos.y - self.size / 2 - camera_y + VIEW_MARGIN_TOP, self.size, self.size))
        else:
            image_rect = image.get_rect(center=(int(self.pos.x - camera_x + VIEW_MARGIN_LEFT), int(self.pos.y - camera_y + VIEW_MARGIN_TOP)))
            screen.blit(image, image_rect)
        if self.selected:
            pygame.draw.rect(screen, self.player_color, (self.pos.x - self.size / 2 - camera_x + VIEW_MARGIN_LEFT, self.pos.y - self.size / 2 - camera_y + VIEW_MARGIN_TOP, self.size, self.size), 1)
...
class Tree(Unit):
    ...
    def draw(self, screen, camera_x, camera_y, axemen_targets):
        if (self.pos.x < camera_x - TILE_SIZE or self.pos.x > camera_x + VIEW_WIDTH + TILE_SIZE or
            ...
            return
        if self._image:
            image_rect = self._image.get_rect(center=(int(self.pos.x - camera_x + VIEW_MARGIN_LEFT), int(self.pos.y - camera_y + VIEW_MARGIN_TOP)))
            tinted_image = self._image.copy()
            if any(self.pos.distance_to(axeman.pos) <= TILE_SIZE and axeman.target == self.pos for axeman in axemen_targets):
                ...
                tint_color = self._colors[self._color_index]
                mask_surface = pygame.Surface(tinted_image.get_size(), pygame.SRCALPHA)
                mask_surface.fill(tint_color + (128,))
                tinted_image.blit(mask_surface, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
            screen.blit(tinted_image, image_rect)
            if any(axeman.target == self.pos for axeman in axemen_targets):
                pygame.draw.rect(screen, WHITE, (self.pos.x - TILE_SIZE / 2 - camera_x + VIEW_MARGIN_LEFT, self.pos.y - TILE_SIZE / 2 - camera_y + VIEW_MARGIN_TOP, TILE_SIZE, TILE_SIZE), 1)
            if self._selected:
                pygame.draw.rect(screen, YELLOW, (self.pos.x - TILE_SIZE / 2 - camera_x + VIEW_MARGIN_LEFT, self.pos.y - TILE_SIZE / 2 - camera_y + VIEW_MARGIN_TOP, TILE_SIZE, TILE_SIZE), 1)
        else:
            pygame.draw.rect(screen, TREE_GREEN, (self.pos.x - TILE_SIZE / 2 - camera_x + VIEW_MARGIN_LEFT, self.pos.y - TILE_SIZE / 2 - camera_y + VIEW_MARGIN_TOP, TILE_SIZE, TILE_SIZE))
...
class Cow(Unit):
    ...
    def draw(self, screen, camera_x, camera_y):
        ...
        bar_height = 4
        bar_offset = 2
        bar_x = self.pos.x - bar_width / 2 - camera_x + VIEW_MARGIN_LEFT
        bar_y = self.pos.y - self.size / 2 - bar_height - bar_offset - camera_y + VIEW_MARGIN_TOP
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_MARGIN_LEFT + VIEW_WIDTH and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_MARGIN_TOP + VIEW_HEIGHT:
                    click_pos = Vector2(mouse_pos.x - VIEW_MARGIN_LEFT + camera_x, mouse_pos.y - VIEW_MARGIN_TOP + camera_y)
                    unit_clicked = None
                    for unit in all_units:
                        if current_player and unit.player_id == current_player.player_id and unit.is_clicked(Vector2(mouse_pos.x - VIEW_MARGIN_LEFT, mouse_pos.y - VIEW_MARGIN_TOP), camera_x, camera_y):
                            unit_clicked = unit
                            break
                    if unit_clicked:
                        for player in players:
                ...
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_MARGIN_LEFT + VIEW_WIDTH and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_MARGIN_TOP + VIEW_HEIGHT:
                    click_pos = Vector2(mouse_pos.x - VIEW_MARGIN_LEFT + camera_x, mouse_pos.y - VIEW_MARGIN_TOP + camera_y)
                    tile_pos = (int(click_pos.x // TILE_SIZE) * TILE_SIZE + TILE_SIZE // 2,
                              int(click_pos.y // TILE_SIZE) * TILE_SIZE + TILE_SIZE // 2)
                    for unit in all_units:
    ...
    for row in range(start_row, end_row):
        for col in range(start_col, end_col):
            ...
            tile_pos = (col * TILE_SIZE + TILE_SIZE // 2, row * TILE_SIZE + TILE_SIZE // 2)
            if tile_pos in move_order_times and current_time - move_order_times[tile_pos] <= 0.4:
                pygame.draw.line(screen, WHITE, (tile_pos[0] - TILE_SIZE // 4 - camera_x + VIEW_MARGIN_LEFT, tile_pos[1] - TILE_SIZE // 4 - camera_y + VIEW_MARGIN_TOP),
                                (tile_pos[0] + TILE_SIZE // 4 - camera_x + VIEW_MARGIN_LEFT, tile_pos[1] + TILE_SIZE // 4 - camera_y + VIEW_MARGIN_TOP), 1)
                pygame.draw.line(screen, WHITE, (tile_pos[0] + TILE_SIZE // 4 - camera_x + VIEW_MARGIN_LEFT, tile_pos[1] - TILE_SIZE // 4 - camera_y + VIEW_MARGIN_TOP),
                                (tile_pos[0] - TILE_SIZE // 4 - camera_x + VIEW_MARGIN_LEFT, tile_pos[1] + TILE_SIZE // 4 - camera_y + VIEW_MARGIN_TOP), 1)
            elif tile_pos in move_order_times and current_time - move_order_times[tile_pos] > 1:
                del move_order_times[tile_pos]
