...
class WaypointGraph:
    def __init__(self, grass_tiles, all_units, tile_size, map_width, map_height, spatial_grid):
        ...
        self.max_distance = 20  # Limit A* search depth for performance

    def is_walkable(self, tile_x, tile_y, unit):
        """Check if a tile is walkable for a unit."""
        ...
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=self.tile_size * 2)  # Increased radius for safety
    ...
    def get_neighbors(self, tile_x, tile_y, unit):
        """Get walkable neighbors (including diagonals)."""
        neighbors = []
        # Include all 8 directions: cardinal and diagonal
        directions = [
        ...
        for dx, dy in directions:
            ...
            if self.is_walkable(new_x, new_y, unit):
                # Assign cost: 1 for cardinal moves, sqrt(2) ≈ 1.414 for diagonal moves
                cost = 1.0 if dx * dy == 0 else 1.414
    ...
    def get_path(self, start_pos, target_pos, unit):
        """Find A* path from start_pos to target_pos or closest reachable point, allowing diagonals."""
...
class Unit:
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        ...
        # Check if target changed or path is exhausted
        if self.target != self.last_target or not self.path or self.path_index >= len(self.path):
