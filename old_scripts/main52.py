...
class WaypointGraph:
    def __init__(self, grass_tiles, all_units, tile_size, map_width, map_height, spatial_grid):
        ...
        self.path_cache = {}  # Cache: (start, target, unit_type) -> path
        self.cache_timeout = 1.0  # Seconds before cache expires
        self.last_cache_clean = time.time()
        self.max_distance = 30  # Increased search depth for better obstacle avoidance

    def is_walkable(self, tile_x, tile_y, unit):
        """Check if a tile is walkable for a unit, considering obstacles."""
        ...
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=self.tile_size * 2)
        for other in nearby_units:
            if isinstance(other, Tree) or (isinstance(other, Building) and not (isinstance(unit, Cow) and isinstance(other, Barn))):
                ...
                if tile_rect.colliderect(unit_rect):
                    return False
        return isinstance(self.grass_tiles[tile_y][tile_x], (GrassTile, Dirt))

    def get_neighbors(self, tile_x, tile_y, unit):
        """Get walkable neighbors (including diagonals) with dynamic obstacle checking."""
        neighbors = []
        directions = [
        ...
        for dx, dy, cost in directions:
            ...
            if self.is_walkable(new_x, new_y, unit):
                # Increase cost near obstacles to prefer paths with more clearance
                obstacle_penalty = self.get_obstacle_penalty(new_x, new_y, unit)
                adjusted_cost = cost * (1.0 + obstacle_penalty)
                neighbors.append((new_x, new_y, adjusted_cost))
        return neighbors

    def get_obstacle_penalty(self, tile_x, tile_y, unit):
        """Calculate a penalty for tiles near obstacles to encourage wider paths."""
        penalty = 0.0
        tile_center = Vector2(tile_x * self.tile_size + self.tile_size / 2, tile_y * self.tile_size + self.tile_size / 2)
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=self.tile_size * 3)
        for other in nearby_units:
            if isinstance(other, Tree) or (isinstance(other, Building) and not (isinstance(unit, Cow) and isinstance(other, Barn))):
                distance = tile_center.distance_to(other.pos)
                if distance < self.tile_size * 2:
                    # Penalty is inversely proportional to distance (closer obstacles increase cost)
                    penalty += max(0.0, 2.0 - (distance / self.tile_size))
        return penalty

    def manhattan_distance(self, tile1, tile2):
        """Calculate Manhattan distance between two tiles (used as heuristic)."""
        return abs(tile1[0] - tile2[0]) + abs(tile1[1] - tile2[1])

    def euclidean_distance(self, tile1, tile2):
        """Calculate Euclidean distance for more accurate diagonal path cost."""
        return math.sqrt((tile2[0] - tile1[0]) ** 2 + (tile2[1] - tile1[1]) ** 2)

    def get_path(self, start_pos, target_pos, unit):
        """Find A* path from start_pos to target_pos or closest reachable point, avoiding obstacles."""
        ...
        # Check cache
        cache_key = (tuple(start_pos), tuple(target_pos), unit.__class__.__name__)
        if cache_key in self.path_cache and current_time - self.path_cache[cache_key]['time'] < self.cache_timeout:
            return self.path_cache[cache_key]['path']
        ...
        # A* algorithm with diagonal movement and obstacle avoidance
        ...
        while open_set:
            ...
            for neighbor_x, neighbor_y, move_cost in self.get_neighbors(current[0], current[1], unit):
                ...
                if tentative_g < g_scores.get(neighbor, float('inf')):
                    ...
                    f_score = tentative_g + self.manhattan_distance(neighbor, target_tile)
        ...
        # Reconstruct path to closest reachable tile
        path = []
        ...
        path = self.smooth_path(path, unit)
    ...
    def smooth_path(self, path, unit):
        """Smooth the path by removing unnecessary waypoints while ensuring obstacle avoidance."""
        if len(path) < 2:
            return path
        ...
        while i < len(path) - 1:
            ...
            next_idx = min(i + 2, len(path) - 1)  # Look ahead to next or next-next point
    ...
    def is_line_of_sight_clear(self, start, end, unit):
        """Check if there's a clear line of sight between two points for the unit."""
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=start.distance_to(end))
        for other in nearby_units:
            if isinstance(other, Tree) or (isinstance(other, Building) and not (isinstance(unit, Cow) and isinstance(other, Barn))):
                unit_rect = pygame.Rect(other.pos.x - other.size / 2, other.pos.y - other.size / 2, other.size, other.size)
...
class Unit:
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        """Move unit along path to target, with fallback for short distances."""
        self.velocity = Vector2(0, 0)
        if not self.target:
            return

        # Handle movement toward a unit target (e.g., enemy for attack)
        if isinstance(self.target, Unit) and not isinstance(self.target, Tree):
            ...
            if distance_to_target <= self.attack_range:
                self.velocity = Vector2(0, 0)  # Stop to attack
                return
            target_pos = self.target.pos
        else:
            target_pos = self.target  # Position target (e.g., grass, Barn corner)

        # Check if target changed, path is exhausted, or obstacles require recalculation
        if (self.target != self.last_target or not self.path or self.path_index >= len(self.path) or
            self.is_path_blocked(units, spatial_grid)):
            self.path = waypoint_graph.get_path(self.pos, target_pos, self) if waypoint_graph else []
            self.path_index = 0
            self.last_target = self.target
            if not self.path:
                # Fallback: move directly if within 2 tiles and clear LOS
        ...
        if self.path_index < len(self.path):
            ...
            if distance > self.size / 2:
                try:
                    ...
                    # Handle case where direction vector is zero (at waypoint)
                    self.path_index += 1
                    if self.path_index >= len(self.path):
                        self.target = None
                        self.path = []
        ...
        # Removed erroneous self.rect.center = self.pos (no rect attribute exists)

    def is_path_blocked(self, units, spatial_grid):
    ...
    def attack(self, target, current_time):
        if not isinstance(target, Unit) or isinstance(target, Tree) or self.attack_damage == 0:
            return False
        if isinstance(target, Building):
            ...
            closest_x = max(target_rect.left, min(self.pos.x, target_rect.right))
            closest_y = max(target_rect.top, min(self.pos.y, target_rect.bottom))
        ...
        if distance <= self.attack_range and current_time - self.last_attack_time >= self.attack_cooldown:
            damage = max(1, self.attack_damage - target.armor)
            target.hp -= damage
            ...
            attack_animations.append({
                ...
                'color': RED if isinstance(self, (Knight, Axeman)) else YELLOW
            })
            print(f"{self.__class__.__name__} at {self.pos} attacked {target.__class__.__name__} at {target.pos}, dealt {damage} damage, target HP now {target.hp}")
            return True
        return False
...
class Axeman(Unit):
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        """Move Axeman to trees, TownCenter for deposit, or set target."""
        self.velocity = Vector2(0, 0)
        if self.special >= 100 and not self.depositing:
            town_centers = [unit for unit in units if isinstance(unit, TownCenter) and unit.player_id == self.player_id and unit.alpha == 255]
            if town_centers:
            ...
            target_unit = next((unit for unit in units if isinstance(unit, TownCenter) and unit.pos == self.target), None)
            if target_unit and self.pos.distance_to(self.target) <= self.size + target_unit.size / 2:
            ...
            if self.pos.distance_to(self.target) <= self.size:
                self.target = None
                self.return_pos = None
...
class Cow(Unit):
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        """Move Cow to grass or Barn autonomously, or follow set target."""
        # Autonomous targeting for grass or Barn
        if self.autonomous_target and not self.target:
            if self.special >= 100:
                # Find a Barn to deposit milk
                barns = [u for u in units if isinstance(u, Barn) and u.player_id == self.player_id and u.alpha == 255]
                if barns:
                    closest_barn = min(barns, key=lambda b: self.pos.distance_to(b.pos))
            ...
            elif self.is_in_barn_any([u for u in units if isinstance(u, Barn)]):
                # Find adjacent grass tile after leaving Barn
                ...
                # Find grass tile if not returning
                tile_x = int(self.pos.x // TILE_SIZE)
                tile_y = int(self.pos.y // TILE_SIZE)
                adjacent_tiles = [
                    (tile_x, tile_y - 1), (tile_x, tile_y + 1), (tile_x - 1, tile_y), (tile_x + 1, tile_y),
        ...
        if self.target and self.pos.distance_to(self.target) <= self.size and self.autonomous_target:
...
class PlayerAI:
    def __init__(self, player, grass_tiles, spatial_grid, all_units, production_queues):
        ...
        self.building_size = 60  # Matches building_size in game loop
    ...
    def find_valid_building_position(self, center_pos):
        ...
        search_radius = 5  # Search within 5 tiles
    ...
    def update(self, current_time):
        """Update AI decisions for Player 2."""
    ...
    def manage_cows(self):
        """Assign cows to harvest grass or deposit milk, respecting 2 cows per barn limit."""
    ...
    def manage_axemen(self):
        """Assign axemen to chop trees or deposit wood, ensuring at least 2 are chopping unless wood is at max."""
        ...
        # Stop harvesting if wood is at or above the limit
        ...
        # Assign idle axemen to trees if needed and wood is below limit
        ...
        # Assign remaining idle axemen to trees
        if self.player.wood < self.player.max_wood:
        ...
        # Assign axemen with wood to deposit
        for axeman in axemen:
    ...
    def manage_buildings(self):
        ...
        # Build barns only if under the limit of 2
        ...
        # Build barracks if no barn is built this cycle and resources are sufficient
    ...
    def manage_unit_production(self):
        """Queue unit production in barns and barracks, maintaining 20% Knights, 40% Axemen, 40% Archers, ensuring 2 Axemen on chopping duty unless wood is at max, and limiting to 2 cows per barn."""
        ...
        total_unit_count = self.player.unit_count  # Includes all non-building units
        chopping_axemen = len([a for a in self.player.units if isinstance(a, Axeman) and a.target and isinstance(a.target, Vector2)])

        # Produce cows in barns if under the limit of 2 cows per barn and total unit limit
        max_cows = barn_count * 2  # Dynamic limit: 2 cows per barn
        ...
        # Produce combat units in barracks
        if barracks and total_unit_count < self.max_units:
            barracks = random.choice(barracks)
            # Calculate desired unit counts based on percentages
            ...
            # Ensure at least 2 Axemen are available for chopping unless wood is maxed
            ...
            # Calculate deficits to prioritize unit production
            axeman_deficit = desired_axemen - axeman_count if total_combat_units > 0 else 1  # Default to wanting Axemen if none exist
            archer_deficit = desired_archers - archer_count if total_combat_units > 0 else 1
            knight_deficit = desired_knights - knight_count if total_combat_units > 0 else 0  # Knights have lowest priority initially

            # Choose unit type with largest deficit
...
while running:
    ...
    for building, queue in list(production_queues.items()):
        ...
        if current_time - queue['start_time'] >= production_time:
            ...
            if not issubclass(queue['unit_type'], Building):
                new_unit = queue['unit_type'](building.pos.x + building.size / 2 + 20, building.pos.y, player.player_id, player.color)
    ...
    player2_ai.update(current_time)
