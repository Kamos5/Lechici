...
class WaypointGraph:
    ...
    def is_walkable(self, tile_x, tile_y, unit):
        """Check if a tile is walkable, using cached results when possible."""
        if not (0 <= tile_x < self.cols and 0 <= tile_y < self.rows):
            return False
        # Check frame-specific walkable cache
        ...
        # Check if tile is grass or dirt
        ...
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=self.tile_size * 1.5)  # Reduced radius
    ...
    def get_path(self, start_pos, target_pos, unit):
        """Find A* path, optimized with caching and reduced search space."""
        # Update frame count for walkable cache
        self.frame_count += 1
        if self.frame_count % 60 == 0:  # Clear frame cache every ~1 second (assuming 60 FPS)
        ...
        # Use tile-based cache key for better reuse
        ...
        # A* algorithm with optimized parameters
        ...
        while open_set:
            ...
            for neighbor_x, neighbor_y, move_cost in self.get_neighbors(current[0], current[1], unit):
                ...
                if tentative_g < g_scores.get(neighbor, float('inf')):
                    ...
                    # Slightly admissible heuristic: scale Manhattan distance for faster convergence
                    ...
                    if dist < closest_distance:
                        ...
                        if dist < self.tile_size / self.tile_size:  # Consider tiles within 1 tile as "close enough"
        ...
        # Simplified path smoothing (less aggressive to save CPU)
