...
GRID_BUTTON_START_X = VIEW_BOUNDS_X - GRID_BUTTON_WIDTH - BUTTON_MARGIN  # Align to right side of panel
...
# Spawn cow button (will be placed in grid[0][0] when barn is selected)
BUTTON_SPAWN_0_POS = (grid_buttons[0][0].x, grid_buttons[0][0].y)
button_spawn_0 = pygame.Rect(BUTTON_SPAWN_0_POS[0], BUTTON_SPAWN_0_POS[1], BUTTON_WIDTH, BUTTON_HEIGHT)
...
class Unit:
    _images = {}
    _unit_icons = {}
    milk_cost = 0  # Default milk cost
    wood_cost = 0  # Default wood cost
...
small_font = pygame.font.SysFont(None, 18)  # Smaller font for grid buttons
...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    ...
                    for player in players:
                        player.deselect_all_units()
                elif barn_selected and current_player and current_player.milk >= Cow.milk_cost and current_player.wood >= Cow.wood_cost and button_spawn_0.collidepoint(event.pos):
                    ...
                    if selected_barn:
                        ...
                        current_player.milk -= new_cow.milk_cost
                        current_player.wood -= new_cow.wood_cost
    ...
    for row in range(GRID_BUTTON_ROWS):
        for col in range(GRID_BUTTON_COLS):
            ...
            if row == 0 and col == 0 and barn_selected and current_player:
                ...
                spawn_cow_text = small_font.render(f"{Cow.milk_cost} milk, {Cow.wood_cost} wood", True, BLACK)
                screen.blit(spawn_cow_text, (button_rect.x + 50, button_rect.y + 12))
            else:
                # Draw empty button
