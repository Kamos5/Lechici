...
class Unit:
    ...
    def draw(self, screen, camera_x, camera_y):
        ...
        if not image:
            ...
            surface.fill(color[:3] + (self.alpha,))  # Apply alpha
            screen.blit(surface, (x - self.size / 2, y - self.size / 2))
        else:
            image_surface = image.copy()
            image_surface.set_alpha(self.alpha)  # Apply alpha
...
class PlayerAI:
    ...
    def manage_axemen(self):
        """Assign axemen to chop trees or deposit wood, ensuring at least 2 are chopping unless wood is at max, and stop harvesting at wood limit."""
        ...
        chopping_axemen = [a for a in axemen if a.target and isinstance(a.target, Vector2)]  # Axemen targeting trees
        depositing_axemen = [a for a in axemen if a.depositing]  # Axemen depositing wood
        num_chopping = len(chopping_axemen)

        # Stop harvesting if wood is at or above the limit (1000)
        if self.player.wood >= self.player.max_wood:
            ...
            num_chopping = 0  # Reset chopping count since all targets are cleared

        # Assign idle axemen to trees if we need more chopping and wood is below limit
        ...
        # Assign remaining idle axemen to trees if wood is below limit
        if self.player.wood < self.player.max_wood:
