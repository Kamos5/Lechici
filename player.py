...
from units import Archer, Axeman, Barn, Barracks, Building, Cow, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman, TownCenter, Tree, Unit, ShamansHut, WarriorsLodge, KnightsEstate, Wall
import context
...
class Player:
    def __init__(self, player_id, color, start_x, start_y):
        ...
        self.used_male_names = set()  # Track used male names for Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman
    ...
    def add_unit(self, unit):
        ...
        # Assign a unique name to Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman, or Cow
        if isinstance(unit, (Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman, Cow)) and unit.name is None:
            if isinstance(unit, Cow) or isinstance(unit, Priestess) or isinstance(unit, Strzyga):
                ...
                # Assign male name to Axeman, Archer, Knight, Bear, Shaman, Swordsman, Spearman
    ...
    def remove_unit(self, unit):
        if unit in self.units:
            ...
            if unit.name:
                ...
                elif isinstance(unit, (Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman)) and unit.name in self.used_male_names:
...
class PlayerAI:
    ...
    def initiate_wave(self, current_time, waypoint_graph):
        ...
        # Wave 2: After 90 seconds, 2 random military units
            ...
            military_units = [u for u in self.player.units if isinstance(u, (Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman)) and u not in self.attack_units]
    ...
    def update_attack_units(self, current_time, waypoint_graph, spatial_grid, all_units, grass_tiles, production_queues):
        ...
        military_units = [u for u in self.player.units if isinstance(u, (Axeman, Archer, Knight, Bear, Strzyga, Priestess, Shaman, Swordsman, Spearman)) and not u.target and u not in self.attack_units]
