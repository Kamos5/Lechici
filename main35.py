...
class Cow(Unit):
    ...
    def __init__(self, x, y, player_id, player_color):
        ...
        self.autonomous_target = False  # New flag to track autonomous movement
        ...
        if self.target:
            ...
            if distance_to_target > self.size:
                ...
                if self.autonomous_target:
                    # Snap to the target tile's center
                    self.pos = Vector2(self.target)
                self.target = None
                self.assigned_corner = None
                self.returning = False
                self.autonomous_target = False  # Reset flag
        self.velocity *= self.damping
        self.pos += self.velocity
    ...
    def harvest_grass(self, grass_tiles, barns, cow_in_barn, player, spatial_grid):
        ...
        if self.special >= 100 and not self.target:
            ...
            if available_barns:
                ...
                self.autonomous_target = True  # Set flag for autonomous movement
...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:
                ...
                if button_player0.collidepoint(event.pos):
                    ...
                    if barn_selected and current_player:
                    ...
                    if not grid_button_clicked and VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                        ...
                        if unit_clicked:
                            ...
                            selecting = True
                            # Inside the game loop, under event.type == pygame.MOUSEBUTTONDOWN, for event.button == 3
            elif event.button == 3:
                mouse_pos = Vector2(event.pos)
                if VIEW_MARGIN_LEFT <= mouse_pos.x <= VIEW_BOUNDS_X and VIEW_MARGIN_TOP <= mouse_pos.y <= VIEW_BOUNDS_Y:
                    click_pos = Vector2(mouse_pos.x - VIEW_MARGIN_LEFT + camera_x, mouse_pos.y - VIEW_MARGIN_TOP + camera_y)
                    # Snap click_pos to the nearest tile center
                    tile_x = int(click_pos.x // TILE_SIZE)
                    tile_y = int(click_pos.y // TILE_SIZE)
                    snapped_pos = (tile_x * TILE_SIZE + TILE_HALF, tile_y * TILE_SIZE + TILE_HALF)  # Use tuple
                    for unit in all_units:
                        if unit.selected and not isinstance(unit, (Building, Tree)):
                            ...
                            if clicked_tree and isinstance(unit, Axeman) and unit.special == 0 and not unit.depositing:
                                ...
                                move_order_times[(clicked_tree.pos.x, clicked_tree.pos.y)] = current_time  # Use tuple
                                print(f"Move order recorded for tree at {(clicked_tree.pos.x, clicked_tree.pos.y)}")
                            else:
    ...
    for row in range(start_row, end_row):
        for col in range(start_col, end_col):
            ...
            tile_pos = (col * TILE_SIZE + TILE_HALF, row * TILE_SIZE + TILE_HALF)  # Use tuple
    ...
    # Draw units
    # First, draw all buildings
    for unit in all_units:
        if isinstance(unit, Building):
            unit.draw(screen, camera_x, camera_y)
    # Then, draw all non-building units (including trees)
    for unit in all_units:
