...
class WaypointGraph:
    ...
    def is_walkable(self, tile_x, tile_y, unit):
        ...
        nearby_units = self.spatial_grid.get_nearby_units(unit, radius=self.tile_size * 1.5)
        for other in nearby_units:
            # Allow Axeman to move onto the tile of their target tree or TownCenter
            if isinstance(unit, Axeman) and unit.target == other.pos and (isinstance(other, Tree) or isinstance(other, TownCenter)):
                continue
...
class Unit:
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        ...
        if isinstance(self.target, Unit) and not isinstance(self.target, Tree):
            ...
            # Use attack range for non-tree units, except TownCenter for Axeman
            stop_distance = self.size + self.target.size / 2 if isinstance(self, Axeman) and isinstance(self.target, TownCenter) else self.attack_range
            if self.pos.distance_to(target_pos) <= stop_distance:
                self.velocity = Vector2(0, 0)  # Stop for TownCenter or to attack
                return
        else:
            target_pos = self.target
            stop_distance = TILE_SIZE if isinstance(self, Axeman) else self.size / 2
        ...
        if self.path_index < len(self.path):
            ...
            if distance > stop_distance:
                try:
                    ...
                    self.path_index += 1
                    if self.path_index >= len(self.path):
                        # Do not clear target for Axeman at tree or TownCenter
                        if isinstance(self, Axeman) and (isinstance(self.target, Vector2) or isinstance(self.target, TownCenter)):
                            self.velocity = Vector2(0, 0)
                        else:
                            self.target = None
                            self.path = []
                ...
                self.path_index += 1
                if self.path_index >= len(self.path):
                    # Stop Axeman at tree or TownCenter
                    if isinstance(self, Axeman) and (isinstance(self.target, Vector2) or isinstance(self.target, TownCenter)):
                        self.velocity = Vector2(0, 0)
                    else:
                        self.target = None
...
class Axeman(Unit):
    ...
    def move(self, units, spatial_grid=None, waypoint_graph=None):
        ...
        # Idle state: No target, look for a tree only if commanded
        ...
        # Returning state: Move back to return_pos after depositing
        if self.target == self.return_pos:
            if self.pos.distance_to(self.target) <= self.size:
                self.target = None
                self.return_pos = None
                print(f"Axeman at {self.pos} reached return position, now idle")
            else:
                super().move(units, spatial_grid, waypoint_graph)
            return
